...
export function VendorResult() {
  ...
  return (
    <ProjectStageShell stageNumber="03" title="能力结果" projectName={viewModel.projectName} outputLabel="设备声明摘要" statusText={viewModel.statusSummary.headline} guidance={{ summary: '您可在本页查看设备能力满足情况、依赖条件与限制说明。' }} statusPanel={<StatusSummaryPanel label={viewModel.statusSummary.title} value={viewModel.statusSummary.headline} note={viewModel.statusSummary.detail} pills={viewModel.statusSummary.pills} />}>
      {({ statusBar }) => (
      <>
      <div className={styles.hero}><div><span className={styles.eyebrow}>设备声明摘要</span><h2>{viewModel.latest.productMeta?.productName || '未命名产品'}</h2><p>{viewModel.latest.productMeta?.deploymentScope || '未填写部署范围'} / SL-{viewModel.latest.productMeta?.securityLevel || 2}</p></div></div>
      ...
      <SectionBlock title="项目能力响应表">
        <DataTable><thead><tr><th>能力要求</th><th>满足状态</th><th>实现方式</th><th>证据类型</th><th>闭环判断</th></tr></thead><tbody>{viewModel.claims.length ? viewModel.claims.map((item) => <tr key={item.capabilityId}><td><strong>{item.label || item.capabilityId}</strong></td><td><StatusBadge tone={item.closureRequired ? 'warning' : 'success'}>{item.statusLabel}</StatusBadge></td><td>{item.implementationTypeLabel}</td><td>{item.evidenceType || '未填写'}</td><td>{item.closureRequired ? '需在闭环页确认责任、条件或补偿' : '可作为当前声明满足项'}</td></tr>) : <tr><td colSpan="5" className={styles.empty}>尚未声明能力项。</td></tr>}</tbody></DataTable>
      </SectionBlock>
      ...
      <div className={styles.actions}><Link to={isReviewMode ? '/report' : '/selection'}><Button variant={isReviewMode ? 'ghost' : 'primary'} size="medium">{isReviewMode ? '返回交付中心' : '进入匹配闭环'}</Button></Link></div>
      </>
      )}
    </ProjectStageShell>
  );
