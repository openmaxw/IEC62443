...
export function VendorCapability() {
  ...
  return (
    <ProjectStageShell stageNumber="03" title="能力声明" projectName={capabilityViewModel.projectName} outputLabel="产品能力声明" statusText={isSummaryStep ? '能力声明已可生成结果页' : '正在完善产品能力声明与证据边界'} statusPanel={<StatusSummaryPanel label="当前步骤" value={`${currentStep + 1} / ${STEPS.length}`} note={validationMessage || (isSummaryStep ? '复核无误后可生成能力声明摘要。' : '点击下一步时会检查当前页必填内容。')} pills={[step.title, `覆盖项目要求 ${claimedRequiredCount}/${requirementRows.length || 0}`]} />} guidance={{ summary: step.guidance }}>
      {({ statusBar }) => (
